...
// Function to display all tuples in a page
void displayPageTuples(const struct Page *page) {
    const char *dataPtr = page->data;
    for (int i = 0; i < page->header.nItems; i++) {
        struct Tuple *tuple = (struct Tuple *)(dataPtr + sizeof(uint16_t)); // Adjust pointer to skip tuple length
        printf("Tuple %d in Page %d:\n", i + 1, page->header.pageNo);
        printf("Name: %s\n", tuple->person.name);
        printf("Address: %s\n", tuple->person.address);
        printf("Age: %d\n\n", tuple->person.age);
        dataPtr += tuple->length;
    }
}
...
int main() {
    ...
    int totalTuples = 10; // Change this to the desired total number of tuples
    ...
    // Display tuples in each page
    struct Page *currentPage = &page;
    while (currentPage != NULL) {
        displayPageTuples(currentPage);
        currentPage = (struct Page *)currentPage->header.nextPageNo;
    }
